...
"""cpu offload for pipeline parallel"""
import warnings
...
def offloading_checker(tensor):
    """Check if the tensor has the offload tag."""
    global _OFFLOAD_TENSOR_MODE
    
    # Only read config on first call
    if _OFFLOAD_TENSOR_MODE is None:
        from megatron.training import get_args
        args = get_args()
        _OFFLOAD_TENSOR_MODE = getattr(args, 'offload_tensors', False)
    
    # If --offload-tensor is not set, return True (offload all tensors)
    if not _OFFLOAD_TENSOR_MODE:
        return True
    
    # If --offload-tensor is set, check if tensor has offload tag
    global OFFLOAD_TAG
    return (
...
def set_offload_tag(tensor):
    """Set the offload tag on the tensor."""
    global OFFLOAD_TAG
...
class _FineGrainedActivationOffloadingNoopManager:
    """No-op manager for callers using the newer grouped offload interface."""
...
class FineGrainedActivationOffloadingInterface:
    """Compatibility adapter for modules expecting grouped activation offload.

    Loong-Megatron's local offload implementation exposes function-style helpers,
    while the DSv4 reference attention modules use a class-style interface. This
    adapter maps to the existing helpers when a chunk handler is active and
    otherwise degrades to no-op behavior.
    """

    _offload_enabled = True

    def __new__(cls, enabled=False, tensor=None, name=None):
        if not enabled or not cls._offload_enabled:
            return _FineGrainedActivationOffloadingNoopManager(tensor)

        cur_forward_chunk = PipelineOffloadManager.get_instance().cur_forward_chunk()
        if cur_forward_chunk is None:
            return _FineGrainedActivationOffloadingNoopManager(tensor)

        return cls._Manager(tensor, name)
    ...
    class _Manager:
        ...
        def __enter__(self):
            if self.tensor is None:
                return self.tensor
            self.tensor = fine_grained_offloading_group_start(self.tensor, name=self.name)
            return self.tensor
        ...
        def group_offload(self, tensor, forced_released_tensors=None):
            ...
            committed = fine_grained_offloading_group_commit(
                tensor, name=self.name, forced_released_tensors=forced_released_tensors
            )
            if isinstance(committed, tuple) and len(committed) == 1:
                return committed[0]
            return committed

    @classmethod
    def disable_offload(cls):
        cls._offload_enabled = False

    @classmethod
    def enable_offload(cls):
        cls._offload_enabled = True
    ...
    @classmethod
    def init_chunk_handler(cls, vp_stage=None, min_offloaded_tensor_size=1024 * 1024):
        fine_grained_offloading_init_chunk_handler(vp_stage, min_offloaded_tensor_size)

    @classmethod
    def mark_not_offload(cls, tensor):
        return tensor

    @classmethod
    def forward_record(cls):
        return None

    @classmethod
    def backward_record(cls, tensor):
        return tensor

    @classmethod
    def enter_replay(cls):
        return None
    ...
    @classmethod
    def flush_delayed_groups(cls):
        return None

    @classmethod
    def cuda_graph_stream(cls):
        return None

    @classmethod
    def cuda_graph_event(cls):
        return None
