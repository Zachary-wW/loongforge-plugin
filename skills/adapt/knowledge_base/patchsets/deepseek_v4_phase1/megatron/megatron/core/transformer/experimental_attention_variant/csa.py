...
def _apply_rope(
    ...
    if rotary_pos_emb is not None and rotary_pos_emb.shape[-1] > pos_dim:
        rotary_pos_emb = rotary_pos_emb[..., -pos_dim:]
    if rotary_pos_cos is not None and rotary_pos_cos.shape[-1] > pos_dim:
        rotary_pos_cos = rotary_pos_cos[..., -pos_dim:]
    if rotary_pos_sin is not None and rotary_pos_sin.shape[-1] > pos_dim:
        rotary_pos_sin = rotary_pos_sin[..., -pos_dim:]
    ...
    if config.apply_rope_fusion:
        ...
        out = _dsv4_apply_rotary_pos_emb(x, rotary_pos_emb, rope_dim=pos_dim)
    if squeeze_head:
        out = out.squeeze(-2)
    return out
...
class Compressor(MegatronModule):
    ...
    def forward(self, x: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        kv = (
            kv * torch.softmax(score, dim=1, dtype=torch.float32).to(kv.dtype)
        ).sum(dim=1)  # [n_compressed, b, head_dim]
...
class CSAIndexer(MegatronModule):
    ...
    -> topk).  DeepSeek-V4 CSA does not apply the DSA Hadamard rotation.
    """
    ...
    def __init__(
        ...
        # Own compressor (smaller head_dim). HF DeepSeek-V4 CSA scores raw
        # RoPE-projected q/k and does not apply the DSA Hadamard rotation.
