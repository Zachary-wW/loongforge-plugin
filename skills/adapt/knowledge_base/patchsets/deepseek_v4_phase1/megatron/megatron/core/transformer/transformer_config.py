...
class TransformerConfig(ModelParallelConfig):
    ...
    """Per-layer DSv4 compressed sparse attention ratios. Valid values are 0, 4, and 128."""

    csa_compress_rotary_base: float = 160000.0
    """RoPE base for compressed KV positions in DSv4 compressed sparse attention."""

    csa_dense_mode: bool = False
    """Disable the learned CSA indexer and attend all compressed positions."""

    apply_dsa_kernel_fusion: bool = False
    """Use fused sparse-attention/indexer kernels when available."""

    o_lora_rank: int = 1024
    """DeepSeek-v4 grouped output projection LoRA rank."""

    o_groups: int = 8
    """Number of DeepSeek-v4 grouped output projection groups."""

    qk_l2_norm: bool = False
    """Compatibility flag used by experimental attention variant specs."""

    qk_clip: Optional[float] = None
    """Compatibility flag; DSv4 hybrid attention does not support QK clipping."""

    mla_down_proj_fusion: bool = False
    """Compatibility flag for MLA down projection fusion."""

    moe_n_hash_layers: int = 0
    """Number of initial MoE layers that use token-id hash routing."""
