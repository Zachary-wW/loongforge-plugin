...
class TransformerLayer(GraphableMegatronModule, BaseTransformerLayer):
    ...
    def _forward_mlp(
        ...
        if self.recompute_mlp:
            ...
            input_id_chunks = [None] * len(chunks)
            if input_ids is not None:
                input_id_chunks = input_ids.transpose(0, 1).contiguous().chunk(num_chunks, dim=0)
                input_id_chunks = [
                    chunk.transpose(0, 1).contiguous() for chunk in input_id_chunks
                ]
            if isinstance(self.mlp, MoELayer):
                outputs = [
                    self.mlp(chunk, input_ids=chunk_input_ids)
                    for chunk, chunk_input_ids in zip(chunks, input_id_chunks)
                ]
            else:
            ...
            if isinstance(self.mlp, MoELayer):
                mlp_output_with_bias = self.mlp(pre_mlp_layernorm_output, input_ids=input_ids)
            else:
