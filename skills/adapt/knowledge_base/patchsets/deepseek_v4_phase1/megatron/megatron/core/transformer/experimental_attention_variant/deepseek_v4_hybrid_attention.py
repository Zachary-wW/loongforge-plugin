...
def _get_rotary_cos_sin(
    ...
    """Return RoPE cos/sin tensors across Megatron rotary API variants."""
    if hasattr(rotary_pos_emb, "get_cached_cos_sin"):
        try:
            return rotary_pos_emb.get_cached_cos_sin(
                seq_len,
                dtype=dtype,
    ...
    rotary_emb = rotary_pos_emb(seq_len, packed_seq=packed_seq)
    if isinstance(rotary_emb, tuple):
        rotary_emb = rotary_emb[0]
    return (
        (rotary_emb.cos() * mscale).to(dtype).contiguous(),
        (rotary_emb.sin() * mscale).to(dtype).contiguous(),
    )


def _dsv4_rotate_half(x: torch.Tensor) -> torch.Tensor:
    x1 = x[..., 0::2]
    x2 = x[..., 1::2]
    return torch.stack((-x2, x1), dim=-1).flatten(-2)


def _dsv4_interleaved_freqs(rotary_pos_emb: torch.Tensor, rope_dim: int) -> torch.Tensor:
    """Return one frequency per DSv4 interleaved pair.

    Megatron's base/Yarn rotary modules store frequencies either as interleaved
    ``[f0, f0, f1, f1, ...]`` (base RoPE) or half-concatenated
    ``[f0, f1, ..., f0, f1, ...]`` (Yarn).  The HF DSv4 implementation keeps
    the half-sized frequency tensor and expands it with ``repeat_interleave(2)``
    at apply time.  Normalize both Megatron layouts to that HF form.
    """
    if rotary_pos_emb.shape[-1] == rope_dim // 2:
        return rotary_pos_emb
    if rotary_pos_emb.shape[-1] > rope_dim:
        half = rotary_pos_emb.shape[-1] // 2
        left, right = rotary_pos_emb[..., :half], rotary_pos_emb[..., half:]
        if torch.equal(left, right):
            return left[..., : rope_dim // 2]
        return rotary_pos_emb[..., :rope_dim:2]
    if rotary_pos_emb.shape[-1] != rope_dim:
        raise ValueError(
            f"Unexpected DSv4 RoPE dim {rotary_pos_emb.shape[-1]} for rope_dim={rope_dim}"
        )
    left, right = rotary_pos_emb[..., : rope_dim // 2], rotary_pos_emb[..., rope_dim // 2 :]
    if torch.equal(left, right):
        return left
    return rotary_pos_emb[..., 0::2]


def _dsv4_apply_rotary_pos_emb(
    x: torch.Tensor,
    rotary_pos_emb: torch.Tensor,
    inverse: bool = False,
    rope_dim: int | None = None,
) -> torch.Tensor:
    """Apply HF-compatible DeepSeek-V4 interleaved RoPE to the trailing rope slice."""
    squeeze_head = x.dim() == 3
    if squeeze_head:
        x = x.unsqueeze(-2)
    rotary_pos_emb = rotary_pos_emb[: x.size(0)]
    if rope_dim is None:
        rope_dim = x.shape[-1]
    freqs = _dsv4_interleaved_freqs(rotary_pos_emb, rope_dim)
    rope_dim = freqs.shape[-1] * 2
    nope, rope = x[..., :-rope_dim], x[..., -rope_dim:]
    cos = freqs.cos().repeat_interleave(2, dim=-1).to(rope.dtype)
    sin = freqs.sin().repeat_interleave(2, dim=-1).to(rope.dtype)
    if inverse:
        sin = -sin
    rotated = (rope.float() * cos) + (_dsv4_rotate_half(rope).float() * sin)
    out = torch.cat([nope, rotated.to(x.dtype)], dim=-1)
    if squeeze_head:
        out = out.squeeze(-2)
    return out
    ...
    def forward(
        ...
        *,
        ...
        if self.config.apply_rope_fusion:
            ...
            rotary_pos_cos, rotary_pos_sin = _get_rotary_cos_sin(
                self.rotary_pos_emb,
                rope_seqlen,
        ...
        if self.config.apply_rope_fusion:
            ...
            core_attn_out = _dsv4_apply_rotary_pos_emb(
                core_attn_out,
                rotary_pos_emb,
                inverse=True,
                rope_dim=pos_dim,
        ...
        *,
        ...
        def qkv_up_proj_and_rope_apply(q_compressed, kv_compressed, k_pos_emb, rotary_pos_emb):
            ...
            if self.config.apply_rope_fusion:
                ...
                q_pos_emb = _dsv4_apply_rotary_pos_emb(q_pos_emb, rotary_pos_emb)
                ...
                k_pos_emb = _dsv4_apply_rotary_pos_emb(k_pos_emb, rotary_pos_emb)
