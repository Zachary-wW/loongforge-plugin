...
Phase 2 does not make HF analysis or network code decisions. It consumes Phase 0's `bridge_mapping.yaml` (primary) or `model_spec.yaml` (legacy fallback) and Phase 1's generated model code and config files from both LoongForge and Megatron repositories.
...
artifacts:
  bridge_mapping_path: phases/phase0/bridge_mapping.yaml  # PRIMARY — weight_map, conversion_requirements, gaps
  model_spec_path: phases/phase0/model_spec.yaml           # legacy fallback when bridge_mapping absent
  hf_analysis_path: phases/phase0/hf_analysis.yaml         # for weight_structure and components (when available)
  reference_contract_path: phases/phase0/reference_contract.yml|null  # DEPRECATED — fields absorbed into bridge_mapping
...
```yaml
artifacts:
  generated_loongforge_files: [...]   # PRIMARY — LoongForge files generated by Phase 1
  generated_megatron_files: [...]     # PRIMARY — Megatron files generated by Phase 1
  generated_files: [...]              # LEGACY fallback (when split fields absent, treat as LoongForge-only)
...
Key path usage rule:
- **bridge_mapping_path is the PRIMARY input** for Phase 2 weight name mapping, conversion requirements, and gap guidance. model_spec_path is a **legacy fallback** used ONLY when bridge_mapping_path is absent.
- **Conversion and roundtrip scripts**: use `phase0_output.slice.hf_ckpt_path` (sliced when available).
- **Dual-repo file consumption**: read `generated_loongforge_files` and `generated_megatron_files` from Phase 1 output. When these split fields are absent (legacy Phase 1 output), fall back to reading `generated_files` as LoongForge-only files.
- **reference_contract_path is DEPRECATED**: its contents (implementation_contract, conversion_requirements, phase3_reference_requirements) have been absorbed into bridge_mapping.yaml (per Phase 6 D-05). Phase 2 no longer reads reference_contract_path.
...
2. If present, invoke `gh_helper.create_branch(owner_repo, branch="adapt/<run_id>/phase2/attempt<K>", base=<base_ref>)` on **both** target repos:
...
3. Record both branch names in `phases/phase2/attempts.jsonl` as `kind="branch"` entries (one per repo).
...
1. If `repos:` block is present, invoke `gh_helper.open_pr(...)` on **both** repos:
   - **LoongForge repo**: `gh_helper.open_pr(owner_repo, head=<branch>, base=<base_ref>, run_id=<run_id>, phase=2, attempt=<K>, kind="base")` with templated title/body.
   - **Megatron repo**: `gh_helper.open_pr(owner_repo, head=<branch>, base=<base_ref>, run_id=<run_id>, phase=2, attempt=<K>, kind="base")`. The Megatron PR body MUST pin the LoongForge commit SHA (VAL-05: `loongforge_commit_sha: <sha>`).
2. Record both PR numbers and URLs in `phases/phase2_output.yml` under the `pr:` block.
...
4. If any PR diff touches protected paths under `references/phases/phase2/verify.md` or `loongforge-phase-gate`, the loop controller will handle escalation to `human_needed` (PR-06).
...
- `weight_structure` section exists (in bridge_mapping or model_spec)
- `bridge_mapping.conversion_requirements` exists (when bridge_mapping_path present), or `model_spec.conversion_requirements` exists as legacy fallback
- Phase 1 generated model code/config files exist (check `generated_loongforge_files` and `generated_megatron_files`, or `generated_files` as legacy fallback)
...
conversion_contract_preflight:
  ...
  forbidden_shortcuts: []
  bridge_mapping_consumed: true|false
```

The `bridge_mapping_consumed` field records whether bridge_mapping was used as the primary input (`true` when bridge_mapping.yaml exists and was read for conversion_requirements; `false` when falling back to model_spec).
...
For gap components (weight_map: null in bridge_mapping), Phase 2 must design new weight mappings. bridge_mapping.gaps[].phase1_guidance may provide clues about the expected module structure.

For Mcore module path discovery, read files from both generated_loongforge_files and generated_megatron_files. Megatron-generated files may introduce new modules with custom weight loading that affects the convert YAML name_map.
...
When bridge_mapping.component_bridge[].weight_map entries exist, use them as the AUTHORITATIVE starting point for name_map generation. Each weight_map entry provides a pre-verified HF→Megatron name correspondence. Cross-verify these against source discovery results — when they agree, accept; when they disagree, prefer source discovery and record the discrepancy.
...
When bridge_mapping.conversion_requirements contains `tp_dimension_overrides`, apply those overrides to the tensor_parallel_dim section. These overrides are authoritative for the model-specific TP dimensions.
...
Invoke `references/tools/code-review/SKILL.md`, execute Phase 2 review (P2-C1 / P2-C2) on all convert YAML + shell files generated in Steps 3-4 (and `tools/convert_checkpoint/` files modified in Step 2, if any), referencing `phase0_output.artifacts.bridge_mapping_path` (primary) or `phase0_output.artifacts.model_spec_path` (legacy fallback) (`weight_structure` section + `components` section).
...
The schema covers step-gate evidence, source/model metadata (including bridge_mapping_path as primary input), generated conversion artifacts (from both LoongForge and Megatron repos), module format mapping, production conversion gate, checks (including bridge_mapping_consumed), and the authoritative `phase2-conversion` validator result.
...
```json
{
  ...
  "details": {
    ...
    "conversion_contract_preflight": {"status": "passed|skipped", "production_required": true, "target_checkpoint_format": "mcore", "bridge_mapping_consumed": true},
...
The `bridge_mapping_consumed` field in `details` records whether bridge_mapping was used as the primary input (`true` when bridge_mapping.yaml existed and was read; `false` when falling back to model_spec).

The `generated_files` list may include both LoongForge and Megatron paths (from `generated_loongforge_files` and `generated_megatron_files`).
