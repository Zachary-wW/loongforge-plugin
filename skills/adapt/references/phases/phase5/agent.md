...
2. If present, invoke `gh_helper.create_branch(owner_repo, branch="adapt/<run_id>/phase5/attempt<K>", base=<base_ref>)` on each target repo. Use `repos.loongforge.url` for `owner_repo` and `repos.loongforge.ref` for `base_ref`.
3. Record the branch name in `phases/phase5/attempts.jsonl` as a `kind="branch"` entry.
...
1. If `repos:` block is present, invoke `gh_helper.open_pr(owner_repo, head=<branch>, base=<base_ref>, run_id=<run_id>, phase=5, attempt=<K>, kind="base")` with templated title/body.
2. Record the PR number and URL in `phases/phase5_output.yml` under the `pr:` block (fields: `number`, `url`).
...
4. If the PR diff touches protected paths under `references/phases/phase5/verify.md` or `loongforge-phase-gate`, the loop controller will handle escalation to `human_needed` (PR-06).
